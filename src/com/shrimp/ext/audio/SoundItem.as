package com.shrimp.ext.audio
{
	...
	/**
	 * Controls playback of a single sound. Comes with convenience methods for all the common Sound APIs (pause, resume, set position, volume etc). This can be used in a modular fashion if all you need
	 * is a simple wrapper around the Sound class.
	 */
	public class SoundItem
	{
		...
		public function SoundItem(sound:Sound = null, type:String = null)
		{
			this.sound = sound;
			this.type = type;
			manager = SoundAS;
		 ...
		 */
		public function play(volume:Number = 1, startTime:Number = 0, loops:int = 0, allowMultiple:Boolean = true, enableSeamlessLoops:Boolean = false):SoundItem
		{
			this.loops = loops;
		 ...
		 */
		public function pause():SoundItem
		{
			if(!channel)
			{
				return this;
		 ...
		 */
		public function resume(forceStart:Boolean = false):SoundItem
		{
		 ...
		 */
		public function stop():SoundItem
		{
			pauseTime = 0;
			stopChannel(channel);
			channel = null;
		 ...
		 */
		public function fadeTo(endVolume:Number, duration:Number = 1000, stopAtZero:Boolean = true):SoundItem
		{
		 ...
		 */
		public function fadeFrom(startVolume:Number, endVolume:Number, duration:Number = 1000, stopAtZero:Boolean = true):SoundItem
		{
		...
		{
			var si:SoundItem = new SoundItem(sound, type);
			return si;
		}
		...
		protected function onSoundComplete(event:Event):void
		{
			...
			if(oldChannels.indexOf(channel) != -1)
			{
				oldChannels.splice(oldChannels.indexOf(channel), 1);
			}
			;
			...
			if(channel == this.channel)
			{
				...
				if(_enableSeamlessLoops == false)
				{
					...
					else
					{
						...
						fireEvent(new SoundEvent(SoundEvent.PLAY_COMPLETE));
					}
				}
				else
				{
		...
		private function fireEvent(e:SoundEvent):void
		{
			if(eventDispatcher.hasEventListener(SoundEvent.PLAY_COMPLETE))
			{
