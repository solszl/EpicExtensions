package com.shrimp.ext.audio
{
	...
	{
		protected var instances:Vector.<SoundItem>;
		 ...
		 */
		public function play(type:String, volume:Number = 1, startTime:Number = 0, loops:int = 0, allowMultiple:Boolean = false, allowInterrupt:Boolean = true, enableSeamlessLoops:Boolean = false):SoundItem
		{
			var si:SoundItem = getSound(type);
		 ...
		 */
		public function playLoop(type:String, volume:Number = 1, startTime:Number = 0, enableSeamlessLoops:Boolean = true):SoundItem
		{
		 ...
		 */
		public function playFx(type:String, volume:Number = 1, startTime:Number = 0, loops:int = 0):SoundItem
		{
		 ...
		 */
		public function resume(type:String):SoundItem
		{
		 ...
		 */
		public function pause(type:String):SoundItem
		{
		 ...
		 */
		public function fadeTo(type:String, endVolume:Number = 1, duration:Number = 1000, stopAtZero:Boolean = true):SoundItem
		{
		 ...
		 */
		public function fadeFrom(type:String, startVolume:Number = 0, endVolume:Number = 1, duration:Number = 1000, stopAtZero:Boolean = true):SoundItem
		{
		 ...
		 */
		public function getSound(type:String, forceNew:Boolean = false):SoundItem
		{
			if(type == null)
			{
				return null;
			...
			var si:SoundItem = instancesByType[type];
		...
		protected function addInstance(si:SoundItem):void
		{
			si.mute = _mute;
			si.manager = this;
		...
		protected function onSoundLoadComplete(event:Event):void
		{
			...
			if(eventDispatcher.hasEventListener(SoundEvent.LOAD_COMPLETE))
			{
				var e:SoundEvent = new SoundEvent(SoundEvent.LOAD_COMPLETE);
		...
		{
			var sound:SoundItem = instancesBySound[event.target as Sound];
			//			loadFailed.dispatch(sound);
			if(eventDispatcher.hasEventListener(SoundEvent.LOAD_FAILED))
			{
				var e:SoundEvent = new SoundEvent(SoundEvent.LOAD_FAILED);
				eventDispatcher.dispatchEvent(e);
			}
